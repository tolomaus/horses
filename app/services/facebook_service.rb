...
class FacebookService
  def register_horse(horse, horse_url, access_token)
    ...
    Rails.logger.info "Calling Facebook server #{uri} with url #{horse_url} to register #{horse.name} (id #{horse.object_id})"
  ...
  def update_horse(horse, access_token)
    ...
    Rails.logger.info "Calling Facebook server #{uri} to update #{horse.name} (id #{horse.object_id})"

    request = Net::HTTP::Post.new(uri.request_uri)
    request.set_form_data({"id" => horse.object_id, "scrape" => "true"})
    response = http.request(request)
    Rails.logger.info "response: #{response.code}: #{response.message}"
    Rails.logger.info "response body: #{response.body}"
    parsed_json = ActiveSupport::JSON.decode(response.body)
